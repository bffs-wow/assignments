...
*   **Trigger Points:** The Google Sheets push logic runs after initial generation and after each refinement turn in the interactive loop (auto-push when creds are present), and via the standalone `push` subcommand (re-pushes the committed assignment set from state without regenerating).
*   **Failure Behavior:** If the Google Sheets API push fails (e.g., network error, API rate limit, invalid credentials), the system issues a loud warning to the user and falls back to the local `.tsv`/`.csv` artifact. The pipeline does not hard-fail.

### 4. Sheet Interaction Strategy

*   **Targeting:** The tool operates on a single encounter's assignment block. All other encounters in the sheet remain untouched. Writes target the **test raid sheet** (per the raid-night scope); the production sheet is out of scope for V1.
*   **Backup:** Before any write operation to an encounter's COUNT block, the existing non-blank rows within that block are read and saved to a local, timestamped CSV backup (e.g., `backups/<encounter_slug>-<YYYYMMDDHHMMSS>.csv`); `backups/` is gitignored.
*   **Block Detection:**
    *   The writer dynamically locates the target encounter's COUNT block by matching **Column B** to the baked sheetName (e.g., `IMMERSEUS`) **and Column D** to `COUNT`.
    *   The data region is the rows immediately following the COUNT header, bounded by the baked per-boss capacity (R1) — the sheet layout is frozen within a tier, so capacity is a baked constant, **not** a runtime scan.
*   **Clear & Replace:** The entire detected COUNT data block for the target encounter is cleared, then the fresh 13-column assignment rows are written starting at the first data row.
*   **Truncation:** If the number of assignments exceeds the baked capacity, they are truncated and a loud warning lists every dropped row.
*   **API requirement:** `values.update` requires `?valueInputOption=RAW` on the PUT path (a bare `values/{range}` path returns `400 INVALID_ARGUMENT`).
*   **Column Handling:** The writer only touches columns A–M in the data region. Columns B, H, I, J, L are generally untouched; Column J (`ADDITIONAL TEXT`) may be written if the AI explicitly generates content for it (human data tends to use K).
